...
describe('the envelope', () => {
  it('stamps the page the event came from', () => {
    ...
    assert.equal(wrapped.attributes['url'], 'https://status.cloudsforge.online/history?x=1')
    assert.equal(wrapped.route, '/history', 'the route is the column Lantern groups on')
    assert.equal(
  ...
  it('carries a kind from Lantern’s closed set, never a bare type', () => {
    installWindow('https://status.cloudsforge.online/')
    const wrapped = envelope({ app: APP_NAME, type: 'NetworkError', message: 'boom' })
  ...
  it('rounds a duration to a whole millisecond, because value_ms is an INTEGER column', () => {
    installWindow('https://status.cloudsforge.online/')
    const wrapped = envelope({ app: APP_NAME, type: 'PageLoad', message: '/', valueMs: 12.7 })
...
describe('the send', () => {
  it('posts to /ingest/client under a "samples" key', async () => {
    ...
    try {
      report({ app: APP_NAME, type: 'NetworkError', message: 'boom' })
      await flush()

 ...
 *
 * ── Why it is here in the ONE repository whose shell does not mount the bar ───────────────────
 *
 * `src/components/shell.tsx` deliberately renders `CloudsForgeLogo` and no `CloudsForgeBar`, and
 * `test/routes.test.ts` holds it to that. Nothing is being reversed here: this file mounts the
 * design system's components DIRECTLY, and says nothing about what this surface's shell renders.
 *
 * It is here because of what follows from that decision. `CloudsForgeLogo` calls no hook, so it
 * renders perfectly well with two Reacts — which means that without this file, nothing in this
 * repository could tell whether the deduplication was working. The `--import` would sit in the
 * `test` script being no help to anybody, and the day this surface takes the bar, the failure
 * would arrive as a novelty. A workaround nothing exercises is a workaround nobody can trust.
 *
